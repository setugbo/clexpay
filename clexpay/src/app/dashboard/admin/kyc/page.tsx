...
export default function AdminKYCUPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: () => fetchKYC(status || undefined, page),
  });
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-wrap items-center gap-4">
            ...
            <Button variant={status === 'pending' ? 'default' : 'outline'} size="sm" onClick={() => { setStatus('pending'); setPage(1); }}>
              Pending
            </Button>
            <Button variant={status === 'verified' ? 'default' : 'outline'} size="sm" onClick={() => { setStatus('verified'); setPage(1); }}>
              Verified
            </Button>
            <Button variant={status === 'rejected' ? 'default' : 'outline'} size="sm" onClick={() => { setStatus('rejected'); setPage(1); }}>
              Rejected
            </Button>
          </div>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-red-600 font-medium">Failed to load KYC data</p>
