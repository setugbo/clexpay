...
export class ManualGiftCardService {
  ...
  async buyGiftCard(userId: string, productId: string, amount: number): Promise<Transaction> {
    ...
    await prisma.activityLog.create({
      data: {
        userId,
        action: 'giftcard_purchase_pending',
  ...
  async generateCardCode(transactionId: string): Promise<string> {
  ...
  async getPendingOrders(): Promise<Transaction[]> {
    ...
    return orders as unknown as Transaction[];
  }
}