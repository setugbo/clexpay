...
export class GiftCardService {
  ...
  private async convertUsdToNgn(usdAmount: number): Promise<number> {
    const { getExchangeRates } = await import('../../factory');
    const rates = await getExchangeRates();
    const usdtRate = rates.USDT_NGN || 1500;
    return Math.round(usdAmount * usdtRate);
  }
  ...
  async getManualQueueOrders(): Promise<Transaction[]> {
    ...
    return all.filter(t => {
      ...
      return meta?.orderStatus === ORDER_STATUS.MANUAL_QUEUE;
    }) as unknown as Transaction[];
  }
  ...
  async getOrderStats(): Promise<{
    ...
    const [total, pending, completed, failed] = await Promise.all([
      ...
      prisma.transaction.count({ where: { type: 'giftcard', status: 'pending' } }),
      prisma.transaction.count({ where: { type: 'giftcard', status: 'success' } }),
      prisma.transaction.count({ where: { type: 'giftcard', status: 'failed' } }),
    ]);

    const pendingTransactions = pending > 0 ? await prisma.transaction.findMany({
      where: { type: 'giftcard', status: 'pending' },
    ...
    return { total, pending, manualQueue, completed, failed, flagged };
  }
}
